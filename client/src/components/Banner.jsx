...
const Banner = () => {
  return (
    ...
    className='flex flex-col md:flex-row md:items-start items-center justify-between px-6 md:px-14 py-10 bg-linear-to-r from-[#0558FE] to-[#A9CFFF] max-w-6xl mx-4 md:mx-auto rounded-2xl overflow-hidden my-16'>
        <div className='text-white flex flex-col items-center md:items-start text-center md:text-left'>
            ...
            <p className='max-w-125 mt-1 opacity-80 text-sm'>We take care of insurance, driver verification and secure payments - so you can earn passive income, stress-free.</p>         
            <motion.button 
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}

            className='px-8 py-2.5 bg-white hover:bg-slate-100 transition-all text-primary font-medium rounded-lg text-sm mt-6 cursor-pointer shadow-lg'>
                List Your Car
            </motion.button>
        </div>
        <motion.img 
        ...
        src={assets.banner_car_image} alt="car" className='w-full max-w-md md:max-w-sm object-contain mt-8 md:mt-0 hover:scale-105 transition-transform duration-500'/>
    </motion.div>
  )
}
